...
from database.db import get_account_limit, get_connected_account_count
...
def cloud_accounts_page():
    ...
    connected_accounts = get_connected_account_count()
    ...
    if account_limit != float('inf'):
        st.progress(min(num_accounts / account_limit, 1.0))
    else:
        st.progress(1.0)
    if num_accounts >= account_limit:
        st.warning(f"Your plan allows up to {account_limit} cloud account(s).\n\n**Upgrade to Growth for 5 accounts and AI insights, or Enterprise for unlimited accounts, automation, and advanced reports!**")
