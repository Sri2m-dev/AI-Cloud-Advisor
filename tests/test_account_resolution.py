...
def test_owner_is_optional_for_approved_business_mapping():
    mapping = complete()
    mapping["owner"] = None
    assert service().allocation_ready(mapping)
    sql = Path("supabase/migrations/202608090002_p42_owner_optional_resolution.sql").read_text(
        encoding="utf-8"
    )
    complete_rule = sql[sql.index("complete :=") : sql.index("target_state :=")]
    assert "requested_mapping->>'owner'" not in complete_rule
