...
begin
 ...
 perform pg_advisory_xact_lock(hashtextextended(concat_ws(':',v_org,v_tenant,'review_created',v_key),0));
 select * into v_existing from data_fabric.stewardship_audit_events where organization_id=v_org and tenant_id=v_tenant and event_type='review_created' and idempotency_key=v_key;
 if found then
  ...
  return jsonb_build_object('review_id',v_existing.review_id,'transition_id',v_existing.event_id,'from_state',v_existing.from_state,'state',v_existing.to_state,'revision',v_existing.resulting_revision,'transitioned_at',v_existing.occurred_at,'idempotency_key',v_existing.idempotency_key,'payload_hash',v_existing.payload_hash,'replayed',true);
 end if;
 ...
 insert into data_fabric.stewardship_audit_events(review_id,organization_id,tenant_id,event_type,to_state,actor,rationale,evidence_references,correlation_id,idempotency_key,payload_hash,resulting_revision)
 values(v_id,v_org,v_tenant,'review_created','discovered',v_actor,p_request->>'rationale',coalesce(v_item->'evidence_references','[]'),v_corr,v_key,v_hash,1)
 returning * into v_existing;
 return jsonb_build_object('review_id',v_id,'transition_id',v_existing.event_id,'from_state',v_existing.from_state,'state',v_existing.to_state,'revision',v_existing.resulting_revision,'transitioned_at',v_existing.occurred_at,'idempotency_key',v_existing.idempotency_key,'payload_hash',v_existing.payload_hash,'replayed',false);
end $$;
...
begin
 ...
 perform pg_advisory_xact_lock(hashtextextended(concat_ws(':',v_org,v_tenant,'state_transition',v_key),0));
 select * into v_existing from data_fabric.stewardship_audit_events where organization_id=v_org and tenant_id=v_tenant and event_type='state_transition' and idempotency_key=v_key;
 if found then
  if v_existing.payload_hash<>v_hash then raise exception 'WP005_IDEMPOTENCY_CONFLICT: key reused with different payload'; end if;
  return jsonb_build_object('review_id',v_existing.review_id,'transition_id',v_existing.event_id,'from_state',v_existing.from_state,'state',v_existing.to_state,'revision',v_existing.resulting_revision,'transitioned_at',v_existing.occurred_at,'idempotency_key',v_existing.idempotency_key,'payload_hash',v_existing.payload_hash,'replayed',true);
 end if;
 ...
 insert into data_fabric.stewardship_audit_events(review_id,organization_id,tenant_id,event_type,from_state,to_state,actor,rationale,evidence_references,correlation_id,idempotency_key,payload_hash,resulting_revision)
 values(v_id,v_org,v_tenant,'state_transition',v_from,v_target,v_actor,p_request->>'rationale',coalesce(p_request->'evidence_references','[]'),v_corr,v_key,v_hash,v_current.revision)
 returning * into v_existing;
 return jsonb_build_object('review_id',v_id,'transition_id',v_existing.event_id,'from_state',v_existing.from_state,'state',v_existing.to_state,'revision',v_existing.resulting_revision,'transitioned_at',v_existing.occurred_at,'idempotency_key',v_existing.idempotency_key,'payload_hash',v_existing.payload_hash,'replayed',false);
end $$;
